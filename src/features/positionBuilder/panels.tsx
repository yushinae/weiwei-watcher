...
// Greeks tab: position greeks cards (1st + higher order), $greeks, per-leg table, hedge hint.
export function PositionGreeksPanel({
  legs, pl, netPremium, probOfProfit, totalSlippage, breakevens, grk,
  dollarGreeks, legGreeksTable, symbol, currentS, maxProfit, maxLoss,
}: {
  legs: Leg[];
  pl: number;
  netPremium: number;
  ...
  totalSlippage: number;
  breakevens: number[];
  grk: { delta: number; gamma: number; theta: number; vega: number; vanna: number; volga: number; charm: number; speed: number };
  dollarGreeks: { dollarDelta: number; dollarGamma: number };
  legGreeksTable: { label: string; delta: number; dollarDelta: number; gamma: number; theta: number; vega: number }[] | null;
  symbol: string;
  currentS: number;
  maxProfit: number | null;
  maxLoss: number | null;
